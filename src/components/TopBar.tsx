    ...
    <header className="border-b border-brand-line bg-white/95 pt-[env(safe-area-inset-top)] shadow-sm backdrop-blur">
      <div className="mx-auto grid min-h-16 w-full max-w-6xl grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-[max(1rem,env(safe-area-inset-left))] py-3 pr-[max(1rem,env(safe-area-inset-right))]">
        <div className="min-w-0">
          ...
          {user ? (
            <p className="mt-0.5 hidden truncate text-xs font-semibold text-zinc-500 sm:block">
  ...
  return (
    <nav aria-label="Mobilnavigation" className="flex items-center justify-end gap-2 sm:hidden">
      ...
      <details className="relative">
        ...
        <div className="absolute right-0 z-50 mt-2 grid min-w-56 gap-2 rounded-2xl border border-zinc-200 bg-white p-2 shadow-xl">
          {user.role === "VC" ? (
