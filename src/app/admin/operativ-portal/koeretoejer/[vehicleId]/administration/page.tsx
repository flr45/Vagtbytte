...
export default async function OperationalVehicleAdministrationPage({ params }: PageProps) {
  ...
  const [vehicle, detail, views, hotspots] = await Promise.all([
    ...
    <OperationalPageFrame>
      ...
      <section className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
        ...
        <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-slate-400">
          Sortér rum og udstyr, flyt flere udstyrsposter mellem rum, lav kopier og opbyg den interaktive 360°-lignende køretøjsnavigation. Billeder, dokumenter og interaktive plusser kopieres ikke automatisk ved kloning.
        </p>
