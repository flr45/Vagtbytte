      ...
      <main className="mx-auto grid w-full max-w-5xl gap-6 px-4 py-6">
        ...
        <section className="rounded-2xl bg-zinc-950 p-6 text-white shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs font-black">
            <span className="rounded-full bg-white/10 px-3 py-2">Antal {item.quantity}</span>
        ...
        <details className="rounded-2xl border border-red-200 bg-red-50 p-5">
          <summary className="cursor-pointer text-lg font-black text-red-950">Redigér udstyr</summary>
          <form action={updateOperationalItemDetailsAction} className="mt-5 grid gap-4 rounded-xl bg-white p-4 md:grid-cols-[minmax(0,1fr)_120px]">
            <input name="itemId" type="hidden" value={item.id} />
            <label className="grid gap-2 text-sm font-bold text-zinc-700">Navn<input className="focus-ring min-h-12 rounded-xl border border-zinc-200 px-4" defaultValue={item.name} name="name" required /></label>
            <label className="grid gap-2 text-sm font-bold text-zinc-700">Antal<input className="focus-ring min-h-12 rounded-xl border border-zinc-200 px-4" defaultValue={item.quantity} min="1" name="quantity" required type="number" /></label>
            <label className="grid gap-2 text-sm font-bold text-zinc-700 md:col-span-2">Note<textarea className="focus-ring min-h-24 rounded-xl border border-zinc-200 p-4" defaultValue={item.note} name="note" placeholder="Placering, kontrolpunkt eller bemærkning" /></label>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-brand-line bg-white p-5 shadow-sm">
            <p className="text-xs font-black uppercase tracking-[0.13em] text-brand-red">Indhold</p>
            <h2 className="mt-1 text-xl font-black">Instruktion og dokumentation</h2>
            <p className="mt-2 text-sm font-semibold text-zinc-600">Tilknyt læringsvideoer, manualer, kontrolskemaer og andre filer direkte til udstyret.</p>
        ...
        <section>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            {videos.length === 0 ? <p className="rounded-2xl border border-dashed border-zinc-300 bg-white p-8 font-semibold text-zinc-600 md:col-span-2">Der er endnu ikke tilknyttet en video.</p> : null}
          </div>
        </section>

        <section>
          <div className="flex items-end justify-between gap-4"><div><h2 className="text-2xl font-black">Dokumenter</h2><p className="mt-1 text-sm font-semibold text-zinc-600">Filer knyttet direkte til {item.name}.</p></div></div>
          <div className="mt-4 overflow-hidden rounded-2xl border border-brand-line bg-white shadow-sm">
            {documents.map((document) => <a className="grid grid-cols-[44px_minmax(0,1fr)_auto] items-center gap-3 border-b border-zinc-100 p-4 last:border-b-0 hover:bg-red-50" href={`/api/admin/operativ-portal/dokumenter/${document.id}`} key={document.id} target="_blank"><span className="grid size-11 place-items-center rounded-lg bg-zinc-950 text-[9px] font-black text-white">FIL</span><span className="min-w-0"><strong className="block truncate">{document.title}</strong><small className="block truncate text-zinc-500">{document.category} · {contentLocationLabel(document)}</small></span><span className="text-xs font-black text-brand-red">Åbn</span></a>)}
            {documents.length === 0 ? <p className="p-8 text-center text-sm font-semibold text-zinc-600">Ingen dokumenter tilknyttet endnu.</p> : null}
          </div>
        </section>
      </main>
    </>
