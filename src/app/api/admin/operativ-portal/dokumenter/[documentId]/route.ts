...
export async function GET(_request: Request, { params }: RouteProps) {
  ...
  try {
    const data = await readFile(
      operationalStoredFilePath(OPERATIONAL_DOCUMENT_DIRECTORY, document.storageName)
    );
    ...
    return new Response(data, {
      headers: {
        ...
        "Content-Disposition": operationalContentDisposition(document.mimeType, encodedName),
        "X-Content-Type-Options": "nosniff"
      }
    });
  } catch {
    return NextResponse.json(
      { error: "Dokumentfilen mangler eller har en ugyldig storage-reference." },
      { status: 410 }
    );
  }
}
